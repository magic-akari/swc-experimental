/// Based on https://github.com/oxc-project/oxc/blob/main/crates/oxc_semantic/src/multi_index_vec.rs
///
...
macro_rules! multi_index_vec {
    (
        ...
        $vis:vis struct $name:ident<$idx:ty> {
            $(
                $fvis:vis $fname:ident => $fname_mut:ident => $fname_unchecked:ident => $fname_mut_unchecked:ident : $fty:ty
            ),* $(,)?
        }
    ) => {
        $(#[$meta])*
        ...
        impl $name {
            ...
            $(
                ...
                #[inline]
                #[allow(dead_code)]
                $fvis unsafe fn $fname_unchecked(&self, id: $idx) -> &$fty {
                    let idx = ::oxc_index::Idx::index(id);
                    unsafe { &*self.$fname.as_ptr().add(idx) }
                }
                ...
                #[inline]
                #[allow(dead_code)]
                $fvis unsafe fn $fname_mut_unchecked(&mut self, id: $idx) -> &mut $fty {
                    let idx = ::oxc_index::Idx::index(id);
                    unsafe { &mut *self.$fname.as_ptr().add(idx) }
                }
            )*
