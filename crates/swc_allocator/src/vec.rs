...
use super::raw_vec::CollectionAllocErr;
...
macro_rules! vec {
    (in $bump:expr; $elem:expr; $n:expr) => {{
        let n = $n;
        let mut v = $crate::vec::Vec::with_capacity_in(n, $bump);
        if n > 0 {
            let elem = $elem;
            for _ in 0..n - 1 {
                v.push(elem.clone());
    ...
    (in $bump:expr) => { $crate::vec::Vec::new_in($bump) };
    (in $bump:expr; $($x:expr),*) => {{
        let mut v = $crate::vec::Vec::new_in($bump);
        $( v.push($x); )*
        v
    }};
    (in $bump:expr; $($x:expr,)*) => ($crate::vec![in $bump; $($x),*])
}
...
impl<T> Vec<'_, T> {
    ...
    const ASSERT_T_IS_NOT_DROP: () = assert!(
        ...
        "Cannot create a Vec<T> where T is a Drop type"
    );
}
...
impl<'bump, T: 'bump> Vec<'bump, T> {
    ...
    /// ```
    #[inline]
    pub fn new_in(bump: &'bump Allocator) -> Vec<'bump, T> {
    ...
    /// ```
    #[inline]
    pub fn with_capacity_in(capacity: usize, bump: &'bump Allocator) -> Vec<'bump, T> {
        const { Self::ASSERT_T_IS_NOT_DROP };
        Vec {
    ...
    /// * `length` needs to be less than or equal to `u32::MAX`, as length is stored as `u32`.
    ...
    pub unsafe fn from_raw_parts_in(
        ...
        const { Self::ASSERT_T_IS_NOT_DROP };
        Vec {
            buf: RawVec::from_raw_parts_in(ptr, length, capacity, bump),
        }
    }
    ...
    #[inline]
    #[must_use]
    pub fn bump(&self) -> &'bump Allocator {
        self.buf.bump()
    }
    ...
    pub fn shrink_to_fit(&mut self) {
        if self.capacity_u32() != self.len_u32() {
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        unsafe {
            ...
            let mut local_len = SetLenOnDrop::new(&mut self.buf);
    ...
    /// # Safety
    ///
    /// - `new_len` must be less than or equal to `u32::MAX`, as length is stored as `u32`.
    ...
    pub unsafe fn set_len(&mut self, new_len: usize) {
        self.buf.set_len(new_len as u32);
    }
    ...
    unsafe fn push_unchecked(&mut self, value: T) {
        ...
        ptr::write(self.buf.ptr().add(self.len_usize()), value);
...
impl<'bump, T> Vec<'bump, T> {
    ...
    pub fn into_boxed_slice(mut self) -> crate::boxed::Box<'bump, [T]> {
        ...
        unsafe {
            let slice = slice::from_raw_parts_mut(self.as_mut_ptr(), self.len_usize());
...
struct SetLenOnDrop<'a, 'bump, T> {
    buf: &'a mut RawVec<'bump, T>,
    local_len: u32,
}

impl<'a, 'bump, T> SetLenOnDrop<'a, 'bump, T> {
    #[inline]
    fn new(buf: &'a mut RawVec<'bump, T>) -> Self {
        SetLenOnDrop {
            local_len: buf.len_u32(),
            buf,
        }
    }
...
impl<T> Drop for SetLenOnDrop<'_, '_, T> {
    fn drop(&mut self) {
        self.buf.set_len(self.local_len);
    }
}
...
impl<'a, 'bump, T> Drain<'a, 'bump, T> {
    ...
    /// # Safety
    ///
    /// Caller must ensure `self.tail_start + self.tail_len <= u32::MAX`,
    /// as that value is passed as a `u32` length to the backing buffer.
