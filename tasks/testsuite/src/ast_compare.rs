...
pub fn ast_compare(ast1: &ast::Program, ast2: &ast::Program) -> Option<String> {
    let nodes1 = collect_node_spans(ast1);
    let nodes2 = collect_node_spans(ast2);

    let mut mismatches = Vec::new();
    for kind in nodes1.keys().chain(nodes2.keys()) {
        let legacy_spans = nodes1.get(kind).map(Vec::as_slice).unwrap_or_default();
        let experimental_spans = nodes2.get(kind).map(Vec::as_slice).unwrap_or_default();
    ...
    for kind in mismatches.iter().take(30) {
        let sample_1 = span_sample(nodes1.get(kind));
        let sample_2 = span_sample(nodes2.get(kind));
        error.push_str(&format!("\n  {kind}: left {sample_1}, right {sample_2}"));
    }
...
type NodeSpans = HashMap<&'static str, Vec<NodeSpan>>;

fn collect_node_spans(program: &ast::Program) -> NodeSpans {
    #[derive(Default)]
    ...
    ($(($method:ident, $kind:literal, $legacy_ty:ident, $experimental_ty:ty)),* $(,)?) => {
        impl Visit for LegacyNodeSpansCollector {
            $(
                fn $method(&mut self, node: &ast::$legacy_ty) {
                    self.nodes.entry($kind).or_default().push(NodeSpan {
    ...
    VisitWith::visit_with(program, &mut collector);
    for spans in collector.nodes.values_mut() {
        spans.sort_unstable();
    }
    collector.nodes
}
